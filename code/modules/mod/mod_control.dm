...
/obj/item/mod/control/proc/get_visor_overlay(mutable_appearance/standing)
	...
	if (length(overrides))
		return overrides[1]
	return mutable_appearance(worn_icon, "[skin]-helmet-visor", layer = HANDS_LAYER + 0.1) // DOPPLER EDIT - return mutable_appearance(worn_icon, "[skin]-helmet-visor", layer = standing.layer + 0.1)
